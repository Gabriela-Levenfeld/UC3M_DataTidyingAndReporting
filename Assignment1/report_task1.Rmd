...
The model classifier showcased a remarkable ability to differentiate between digits $4$ and $9$, achieving an accuracy of `r percentage_49`%. in this case, it is possible to identify the existence of two regions (corresponding to the blue area) whose relevance is greater. **Buscar 2 imágenes para ver a qué zonas puede corresponder estas zonas**
...
Classify one digit versus another involves the creation of $45$ different models. This approach yielded an incredible average accuracy of `r average_accuracy`%. Among these, the best performing was between digits `r highest_position[1]` and `r highest_position[2]`, achieving an accuracy of `r percentage_best`%. On the other hand, even the worst model obtained a `r percentage_lowest`% of accuracy which was obtained by digits `r lowest_position[1]` and `r lowest_position[2]`.
