...
#' install.packages(c("glmnet", "ggplot2", "reshape2"))
```
...
**$\text{\underline{4 vs 9 classifier}}$**.
...
The model classifier showcased a remarkable ability to differentiate between digits $4$ and $9$, achieving an accuracy of `r result_49$percentage`%. Two critical regions are identified over the blue areas. The left region, in particular, captures the curve of the digit $9$, contrasting with the peak characteristic of the digit $4$. 

```{r classification_4vs9_plot, echo=FALSE, out.width="50%"}
plot_beta_coef(result_49$lambda, digit_a, digit_b, train_nist)
visualize_beta_ranks(beta_ranks_49)
```

**$\text{\underline{General classifier}}$**.
...
# Highes accuracy and its position
...
# Find the lowest accuracy and its position (excluding zeros)
...
percentage_average <- average_accuracy*100
...
```{r precomputed_beta_ranks, echo=FALSE, out.width="50%"}
...
Classify one digit versus another involves the creation of $45$ different models. This approach yielded an incredible average accuracy of `r percentage_average`%. Among these, the best performing was between digits `r highest_position[1]` and `r highest_position[2]`, achieving an accuracy of `r percentage_best`%. On the other hand, even the worst model obtained a `r percentage_lowest`% of accuracy which was obtained by digits `r lowest_position[1]` and `r lowest_position[2]`.
...
The ridge logistic model has demonstrated an outstanding performance across all tasks, distinguishing greatly between pairs of digits. Therefore, it can be concluded that the chosen model is a great election for this classification problem.
Lastly, it is important to mention that the report only include the main insight found as well as a comprehensive explanation. For more detailed information, please refer to the RMarkdown script with the same name.
